...
public class Eyes : MonoBehaviour {
    ...
    [HideInInspector]
    public float dis_l, dis_flO, dis_flT, dis_f, dis_frO, dis_frT, dis_r, dis_b = 0f;
    [HideInInspector]
	...
	private void Update () {
		Vector3 originPosition = transform.position + Vector3.up * 0.5f;
        float originRotation = transform.rotation.eulerAngles.y - 180f;
        float angle = originRotation / 180 * Mathf.PI;

		Vector3 front = new Vector3 (originPosition.x - raycastLength * Mathf.Sin (angle), originPosition.y, originPosition.z - raycastLength * Mathf.Cos (angle));
		Vector3 back = originPosition + originPosition - front;

		Vector3 right = new Vector3 (originPosition.x - raycastLength * Mathf.Cos (angle), originPosition.y, originPosition.z + raycastLength * Mathf.Sin (angle));
		Vector3 frontRightOne = new Vector3 (originPosition.x - raycastLength * Mathf.Sin (angle - Mathf.PI / 4), originPosition.y, originPosition.z - raycastLength * Mathf.Cos (angle - Mathf.PI / 4));
		Vector3 frontRightTwo = new Vector3 (originPosition.x - raycastLength * Mathf.Sin (angle - Mathf.PI / 8), originPosition.y, originPosition.z - raycastLength * Mathf.Cos (angle - Mathf.PI / 8));

		Vector3 left = originPosition + originPosition - right;
		Vector3 frontLeftOne = new Vector3 (originPosition.x - raycastLength * Mathf.Sin (angle + Mathf.PI / 4), originPosition.y, originPosition.z - raycastLength * Mathf.Cos (angle + Mathf.PI / 4));
		Vector3 frontLeftTwo = new Vector3 (originPosition.x - raycastLength * Mathf.Sin (angle + Mathf.PI / 8), originPosition.y, originPosition.z - raycastLength * Mathf.Cos (angle + Mathf.PI / 8));
